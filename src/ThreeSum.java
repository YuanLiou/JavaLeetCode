...
public class ThreeSum {
    public static void main(String[] args) {
        ...
        var result = threeSum(sampleTest);
    ...
    public static List<List<Integer>> threeSum(int[] nums) {
        ...
        //  1-a. for loop i..<nums.length - 2。要找三個數字，length - 1 沒意義
        //  如果現在 i > 0 並且他前面那個數字跟他不一樣，再下去比大小 (不然可能找到重複的）
        //  2-a. while loop (lowerBound < upperBound)
        //  lowerBound = i + 1, upperBound = nums.length - 1
        //  尋找目標 x = 0 - num[i]
        //  3-a. num[i] + num[lowerBound] + num[upperBound] = x
        //  這樣就是找到，加到 Result 列表
        //    3-a-i. 這邊要注意，如果 lowerBound 和 lowerBound + 1 位子的數字相同，
        //           先把 lowerBound 移到和下一個數字不同的位置，以免重複計算
        //   3-a-ii. 這邊要注意，如果 upperBound 和 upperBound - 1 位子的數字相同，
        //           先把 upperBound 移到和上一個數字不同的位置，以免重複計算
        //  3-a-iii. 切記這些都要確保 lowerBound < upperBound
        //  3-b. num[i] + num[lowerBound] + num[upperBound] > x
        //  太大了，upperBound 往裡面調整試試 (已排序，最右邊最大）
        //  3-c. num[i] + num[lowerBound] + num[upperBound] < x
        //  太小了，lowerBound 往裡面調整試試 (已排序，最左邊最小）
